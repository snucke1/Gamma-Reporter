...
def calculate_gamma_profile(df, from_strike, to_strike):
    ...
    total_gamma = []
    for level in levels:
        call_gamma = df.apply(lambda row: calcGammaEx(level, row['StrikePrice'], row['CallIV'], row['daysTillExp'], CONFIG['RISK_FREE_RATE'], CONFIG['DIVIDEND_YIELD'], row['CallOpenInt']), axis=1)
        put_gamma = df.apply(lambda row: calcGammaEx(level, row['StrikePrice'], row['PutIV'], row['daysTillExp'], CONFIG['RISK_FREE_RATE'], CONFIG['DIVIDEND_YIELD'], row['PutOpenInt']), axis=1)
