...
# <-- FIX: Added a dedicated validation function for the profile chart's data needs.
def validate_and_clean_profile_data(df):
    # This version does not require 'CallGamma' or 'PutGamma' as they are not used.
...
def process_gamma_reporter(df, spot_price):
...
# --- PLOTTING FUNCTION (Original Reporter) ---
def plot_gamma_reporter(df, spot_price, from_strike, to_strike, index, title_prefix, ax1, ax2, bin_width=None, add_guidance_box=False):
    # This function remains unchanged, only renamed for clarity
    ...
    ax1.legend()
    # Guidance box logic remains the same...
...
# --- FUNCTIONS FOR GAMMA PROFILE PLOT ---
...
def plot_gamma_profile(df, spot_price, index):
    ...
    for level in levels:
        ...
        # Dealers are short options, so we subtract both call and put gamma exposure from their perspective
        # However, for market GEX, we consider puts as negative exposure.
...
def main():
    ...
    # --- Prepare 0-1 DTE Data ---
    unique_expiries = sorted(data_df['ExpirationDate'].unique())
    if len(unique_expiries) < 2:
        logging.error("Not enough expiration dates for 0-1 DTE analysis."); sys.exit(1)
    target_expiries = unique_expiries[:2]
    
    data_0dte = data_df[data_df['ExpirationDate'].isin(target_expiries)]
    ...
    df_0dte_merged = calls_0dte.merge(puts_0dte, on=['ExpirationDate', 'Strike'], suffixes=('_call', '_put'))
    
    # --- Report 1: Gamma Reporter Chart ---
    df_reporter = df_0dte_merged.rename(columns={
        'Strike': 'StrikePrice', 'iv_call': 'CallIV', 'gamma_call': 'CallGamma', 'open_interest_call': 'CallOpenInt',
        'iv_put': 'PutIV', 'gamma_put': 'PutGamma', 'open_interest_put': 'PutOpenInt'})
    df_reporter = validate_and_clean_reporter_data(df_reporter)
    if len(df_reporter) > 0: df_reporter = process_gamma_reporter(df_reporter, spot_price)

    # --- Report 2: Gamma Profile Chart ---
    # <-- FIX: Use a clean dataframe specifically for the profile, as it has different column names needed for validation.
    df_profile = df_0dte_merged.rename(columns={
        'Strike': 'StrikePrice', 'iv_call': 'CallIV', 'open_interest_call': 'CallOpenInt',
        'iv_put': 'PutIV', 'open_interest_put': 'PutOpenInt'})
    df_profile['ExpirationDate'] += timedelta(hours=16)
    df_profile = validate_and_clean_profile_data(df_profile) # <-- FIX: Call the new validation function.
    ...
    if len(df_reporter) > 0:
        plot_gamma_reporter(df_reporter, spot_price, from_strike_0dte, to_strike_0dte, index, "0-1DTE", axs[0,0], axs[1,0])
    # The 6M logic can be added back here if needed, but the focus is fixing 0DTE
    ...
    if len(df_profile) > 0:
        chart2_filename = plot_gamma_profile(df_profile, spot_price, index)
    else:
        logging.warning("Skipping gamma profile chart due to no valid 0-1DTE profile data.")

    charts_to_send = [p for p in [chart1_filename, chart2_filename] if p is not None]
    if charts_to_send:
